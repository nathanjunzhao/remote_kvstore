...
/**
 * @brief computes the hash of the key `key`
 *
 * Computes the Fowler / Noll / Vo (FNV) hash of the string `key`.
 *
 * @param key a string of length `KEYSIZE` as defined in hash_table.h
 */
static unsigned int
...
hash_table_resize(struct hash_table *ht, unsigned int new_size)
{
	...
	for (index = 0; index < ht->size; index++) {
		 ...
		 */
		for (elem = ht->head[index]; elem != NULL;) {
			/*
...
hash_table_insert(struct hash_table *ht, const char *key, void *value)
{
	...
	if (hash_table_lookup(ht, key) == NULL) {
		...
		/*
		 * Compute the array index of the collision chain in which the
		...
		for (elem = ht->head[index]; elem != NULL; elem = elem->next) {
			/*
			 * If "key" matches the element's key, then replace the
			 * element's value.
			 */
			if (memcmp(elem->key, key, KEYSIZE) == 0) {
				// Update key to this instance to be safe
				elem->key = key;
				// Overwrite value
				elem->value = value;
			}
...
hash_table_iterate(struct hash_table *ht,
    void *(*function)(const char *key, void *value, void *cookie), void *cookie)
{
	struct hash_table_mapping *elem;
	unsigned int		   index;
    ...
    void *cookie)
{
	 ...
	 * Iterate over that collision chain.
	 */
	struct hash_table_mapping *current = NULL;
	for (elem = ht->head[index]; elem != NULL; elem = elem->next) {
		/*
